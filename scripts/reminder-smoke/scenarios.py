...
def card(name: str) -> dict:
    ...
    with db_connection() as c:
        ...
        start = (datetime.now(UTC) + timedelta(hours=8)) if name == "catch-up" else (datetime.now(UTC) + (timedelta(minutes=30 + FIXTURE_NAMES.index(name) * 40) if name != "overdue" else timedelta(hours=-2)))
