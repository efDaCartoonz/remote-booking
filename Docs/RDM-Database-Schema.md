...
> **Актуальность:** эта схема описывает первую миграцию, а не текущий
> migration head. Последующие таблицы и ограничения описаны в
> Alembic (`backend/alembic/versions/`); текущий head —
> `20260924_0012`. Статусы этапов см. `Docs/RDM-ROADMAP.md`.
...
`outbox_events` не входил в первую схему MVP. Последующие миграции
добавили специализированный `omnidesk_outbox`; текущее устройство очереди
определяется миграциями и реализацией IE-01.
