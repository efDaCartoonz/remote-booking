...
class ReminderService:
    def __init__(
        self, repository: ReminderRepository, notifications: NotificationService
    ):
    ...
    def scan(self, *, now: datetime, batch_size: int) -> int:
        ...
        created = 0
        for reminder in self.repository.claim_due(
            now=now, limit=min(max(batch_size, 1), 500)
        ):
            try:
                with getattr(self.repository, "savepoint", nullcontext)():
                    created += self._scan_one(reminder, now)
            except Exception as exc:  # noqa: BLE001 - one schedule must not block a batch
                logger.warning(
                    "reminder_scan_failed id=%s error=%s",
                    reminder.id,
                    type(exc).__name__,
                )
        return created
    ...
    def _scan_one(self, reminder: DueReminder, now: datetime) -> int:
        ...
        if not self.repository.current(reminder):
            ...
            return 0
        elapsed = max(
            0, int((now - reminder.anchor_at.astimezone(UTC)).total_seconds())
        )
        count = max(reminder.last_count + 1, elapsed // reminder.interval_seconds)
        event_id = self.repository.record_timer_event(
            card_id=reminder.card_id, kind=reminder.kind, count=count
        )
        for channel, _ in self.repository.recipients(user_id=reminder.owner_id or 0):
            if self.notifications.notify(
                ...
                source_event_type=4,
                recipient_user_id=reminder.owner_id or 0,
                channel=channel,
                payload={
                    "card_id": reminder.card_id,
                    "assignment": "l2" if reminder.kind == "l2_reminder" else "l1",
                },
            ):
                created += 1
        ...
        self.repository.advance(
            reminder_id=reminder.id,
            next_due_at=reminder.anchor_at
            + timedelta(seconds=(count + 1) * reminder.interval_seconds),
            count=count,
            escalation_sent=reminder.escalation_sent or escalate,
            last_escalated_at=now if escalate else reminder.last_escalated_at,
        )
        return created
...
class PostgresReminderRepository:
    ...
    def record_timer_event(self, *, card_id: int, kind: str, count: int) -> int:
        with self.connection.cursor() as cursor:
            cursor.execute(
                "INSERT INTO card_events (card_id, event_type_code, actor_type_code, new_values, comment) VALUES (%(card)s, 4, 2, %(values)s, %(comment)s) RETURNING id",
                {
                    "card": card_id,
                    "values": Jsonb({"timer": kind, "count": count}),
                    "comment": "timer_reminder",
                },
            )
            event_id = int(cursor.fetchone()["id"])
            cursor.execute(
                "INSERT INTO audit_log (actor_type_code, action_code, entity_type, entity_id, new_values) VALUES (2, 1, 'reminder_schedule', %(card)s, %(values)s)",
                {
                    "card": card_id,
                    "values": Jsonb(
                        {"kind": kind, "count": count, "event_id": event_id}
                    ),
                },
            )
            return event_id
    ...
    def recipients(self, *, user_id: int) -> list[tuple[str, str]]:
        with self.connection.cursor() as cursor:
            cursor.execute(
                "SELECT telegram_chat_id, bitrix24_user_id FROM user_settings WHERE user_id = %(id)s",
                {"id": user_id},
            )
            row = cursor.fetchone()
        return (
    ...
    def managers(self) -> list[tuple[int, str, str]]:
        with self.connection.cursor() as cursor:
            cursor.execute(
                "SELECT u.id, us.telegram_chat_id, us.bitrix24_user_id FROM users u JOIN user_roles ur ON ur.user_id=u.id AND ur.role_id=3 LEFT JOIN user_settings us ON us.user_id=u.id WHERE u.is_active ORDER BY u.id"
            )
            return [
                (r["id"], channel, value)
    ...
    def advance(
        ...
        *,
        ...
        with self.connection.cursor() as cursor:
            cursor.execute(
                "UPDATE reminder_schedules SET next_due_at=%(due)s, last_count=%(count)s, escalation_sent=%(sent)s, last_escalated_at=%(last)s, closed_at=CASE WHEN %(close)s THEN now() ELSE closed_at END WHERE id=%(id)s AND closed_at IS NULL",
                {
                    "id": reminder_id,
                    "due": next_due_at,
                    "count": count,
                    "sent": escalation_sent,
                    "last": last_escalated_at,
                    "close": close,
                },
            )
