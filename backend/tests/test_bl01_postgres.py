...
def test_omnidesk_reschedule_persists_l1_closure_and_deduplicates(database_url):
    with psycopg.connect(database_url, row_factory=dict_row) as connection:
        ...
        with connection.cursor() as cursor:
            cursor.execute("UPDATE distribution_members SET is_enabled=true WHERE user_id IN (92002,92003)")
        ...
        service.apply_confirmed_omnidesk_reschedule(command)
        with connection.cursor() as cursor:
            cursor.execute("SELECT status_code, l1_owner_id, planned_duration_minutes FROM connection_cards WHERE id=%s", (rejected.id,))
            ...
            assert cursor.fetchone()["count"] == 1
            cursor.execute("SELECT count(*) FROM reminder_schedules WHERE card_id=%s AND kind='l2_reminder' AND closed_at IS NULL", (rejected.id,))
            assert cursor.fetchone()["count"] == 1
            cursor.execute("SELECT count(*) FROM assignment_cycles WHERE card_id=%s", (rejected.id,))
            assert cursor.fetchone()["count"] == 2
            cursor.execute("SELECT count(*) FROM assignment_attempts WHERE card_id=%s AND status_code=0", (rejected.id,))
            assert cursor.fetchone()["count"] == 1
        ...
        with connection.cursor() as cursor:
            cursor.execute("SELECT count(*) FROM card_events WHERE card_id=%s AND comment LIKE 'omnidesk_rescheduled:%%'", (rejected.id,))
            assert cursor.fetchone()["count"] == 1


def test_manager_reassignment_persists_cycle_attempt_and_reminder_lifecycle(database_url):
    with psycopg.connect(database_url, row_factory=dict_row) as connection:
        _seed(connection)
        service = CardService(PostgresCardRepository(connection))
        ...
        with connection.cursor() as cursor:
            cursor.execute("SELECT cycle_number, status_code FROM assignment_cycles WHERE card_id=%s ORDER BY cycle_number", (card.id,))
            assert [dict(row) for row in cursor.fetchall()] == [
                {"cycle_number": 1, "status_code": 3},
                {"cycle_number": 2, "status_code": 1},
            ]
            cursor.execute("SELECT l2_engineer_id, status_code, actor_user_id FROM assignment_attempts WHERE card_id=%s ORDER BY id", (card.id,))
            assert [dict(row) for row in cursor.fetchall()] == [
                {"l2_engineer_id": 92002, "status_code": 3, "actor_user_id": 92000},
                {"l2_engineer_id": 92003, "status_code": 0, "actor_user_id": None},
            ]
            cursor.execute("SELECT count(*) FROM reminder_schedules WHERE card_id=%s AND kind='l2_reminder' AND closed_at IS NOT NULL", (card.id,))
            assert cursor.fetchone()["count"] == 1
            cursor.execute("SELECT count(*) FROM reminder_schedules WHERE card_id=%s AND kind='l2_reminder' AND closed_at IS NULL", (card.id,))
            assert cursor.fetchone()["count"] == 1
            cursor.execute("SELECT count(*) FROM card_events WHERE card_id=%s AND comment='manager_manual_assignment'", (card.id,))
            assert cursor.fetchone()["count"] == 1
            cursor.execute("SELECT count(*) FROM audit_log WHERE entity_type='assignment_cycle' AND actor_user_id=92000", ())
            assert cursor.fetchone()["count"] == 1
...
def test_l2_overdue_persists_l1_followup_and_deduplicates_escalation(database_url):
    with psycopg.connect(database_url, row_factory=dict_row) as connection:
        ...
        with connection.cursor() as cursor:
            ...
            cursor.execute("UPDATE reminder_schedules SET next_due_at=%s WHERE card_id=%s AND kind='l2_reminder' AND closed_at IS NULL", (now - timedelta(minutes=1), card.id))
        ...
        assert scanner.scan(now=now, batch_size=10) == 0
        with connection.cursor() as cursor:
            cursor.execute("SELECT overdue_at, overdue_flag, l1_owner_id, status_code FROM connection_cards WHERE id=%s", (card.id,))
